...
function onCursorUpdate() {
  ...
  if (state.selection.main.from != state.selection.main.to) {
    ...
    window.dispatchEvent(new CustomEvent("copilot:editor:select", {
      detail: {
        selection: state.sliceDoc(state.selection.main.from, state.selection.main.to),
    ...
    if (newLine || col == line.text.length && line.text[col - 1] == " ") {
      ...
      window.dispatchEvent(new CustomEvent("copilot:editor:update", {
        detail: {
          prefix: prefix.join(" "),
          pos,
          newLine,
...
function onAcceptImprovement(e: CustomEvent<{ improvement: string, from: number, to: number }>) {
  var editor = document.querySelector(".cm-content");
  if (!editor) return;
  ...
  if (state.selection.main.from == e.detail.from && state.selection.main.to == e.detail.to) {
    const changes = { from: e.detail.from, to: e.detail.to, insert: e.detail.improvement };
    const selection = { anchor: e.detail.from + e.detail.improvement.length };
    content.cmView.view.dispatch({ changes, selection });
  }
}

window.addEventListener("copilot:editor:replace", onAcceptImprovement as EventListener);
window.addEventListener("cursor:editor:update", debounce(onCursorUpdate));
window.addEventListener("copilot:config:update", onConfigUpdate as EventListener);
document.querySelector(".cm-content")?.addEventListener("keydown", onKeyDown as EventListener);