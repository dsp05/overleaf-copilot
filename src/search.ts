...
(() => {
  ...
  async function load(query: string, page: number) {
    const container = document.getElementById("search-result-container") as HTMLElement;
    container.replaceChildren();

    const loadingTemplate = document.getElementById("loading") as HTMLTemplateElement;
    ...
    if (ids.length == metadata.length) {
      container.replaceChildren();
      const template = document.getElementById("search-result") as HTMLTemplateElement;
    ...
    const paginationTemplate = document.getElementById("pagination") as HTMLTemplateElement;
    const paginationClone = document.importNode(paginationTemplate.content, true);
