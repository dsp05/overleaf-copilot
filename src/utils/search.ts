...
export async function search(query: string, page: number) {
  ...
  const data = new Map<
    string,
    { id: string; data: { text: string; score: number }[] }
  >();
  const ids: string[] = [];
  for (const c of content) {
    if (!c.payload.id || !c.payload.text) continue;
    if (data.has(c.payload.id))
      data
        .get(c.payload.id)!
        .data.push({ text: c.payload.text, score: c.score });
    else {
      data.set(c.payload.id, {
        id: c.payload.id,
        data: [{ text: c.payload.text, score: c.score }],
      });
