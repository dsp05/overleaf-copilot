...
import { CONFIG_DISABLE_COMPLETION, CONFIG_DISABLE_IMPROVEMENT, CONFIG_MAX_PROMPT_WORDS } from "./constants";

let cursorPos: { row: number, column: number } | null = null;

function debounce<T extends (content: string, pos: number, row: number, col: number, signal: AbortSignal)
  => Promise<void>>(func: T): EventListener {
  ...
  return async function (event: Event) {
    ...
    const detail = (event as CustomEvent<{ prefix: string, pos: number, row: number, col: number }>).detail;
    await func(detail.prefix, detail.pos, detail.row, detail.col, controller.signal);
  }
}

async function onEditorUpdate(content: string, pos: number, row: number, col: number, signal: AbortSignal) {
  ...
  suggestion.onclick = () => document.getElementById("copilot-suggestion")?.remove();
...
async function onConfigUpdate() {
  ...
  window.dispatchEvent(new CustomEvent("copilot:config:update", { detail: config }));
}

async function onEditorSelect(event: CustomEvent<{ selection: string, from: number, to: number, head: number }>) {
  ...
  if (event.detail.to - event.detail.head < event.detail.head - event.detail.from) {
  ...
  button.style.backgroundImage = `url("${chrome.runtime.getURL("icons/icon_128.png")}")`;
  button.onclick = async () => {
    document.getElementById("copilot-sidebar-button")?.remove();
    document.getElementById("copilot-side-panel")?.remove();
    await LoadSidePanel(event.detail.selection, event.detail.from, event.detail.to);
  };
...
window.addEventListener("cursor:editor:update", onCursorUpdate as EventListener);
