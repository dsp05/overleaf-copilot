...
(function () {
  function restoreConfig() {
    chrome.storage.local.get(
      ...
      (result) => {
        ...
        if (!!baseURL) {
          document
            .getElementById(CONFIG_BASE_URL)!
            .setAttribute('placeholder', baseURL);
        }
        ...
        const maxTokenInput = document.getElementById(
          CONFIG_MAX_TOKEN
        ) as HTMLInputElement;
        maxTokenInput.value = result[CONFIG_MAX_TOKEN] ?? '';
        maxTokenInput.disabled = disableCompletion;

        const maxPromptWordsInput = document.getElementById(
          CONFIG_MAX_PROMPT_WORDS
        ) as HTMLInputElement;
        maxPromptWordsInput.value = result[CONFIG_MAX_PROMPT_WORDS] ?? '';
        maxPromptWordsInput.disabled = disableCompletion;

        const completionCustomPromptInput = document.getElementById(
          CONFIG_COMPLETION_CUSTOM_PROMPT
        )! as HTMLInputElement;
        completionCustomPromptInput.value =
          result[CONFIG_COMPLETION_CUSTOM_PROMPT] ?? '';
        ...
        ).checked = disableImprovement;

        const improvementCustomPromptInput = document.getElementById(
          CONFIG_IMPROVEMENT_CUSTOM_PROMPT
        ) as HTMLInputElement;
        improvementCustomPromptInput.value =
          result[CONFIG_IMPROVEMENT_CUSTOM_PROMPT] ?? '';
  ...
  document.getElementById('btn-save')!.addEventListener('click', async () => {
    const model = (document.getElementById(CONFIG_MODEL)! as HTMLSelectElement)
      .value;
    const apiKey = (
      document.getElementById(CONFIG_API_KEY)! as HTMLInputElement
    ).value;
    const baseURL = (
      document.getElementById(CONFIG_BASE_URL)! as HTMLInputElement
    ).value;
    const maxToken = (
      document.getElementById(CONFIG_MAX_TOKEN)! as HTMLInputElement
    ).value;
    const maxPromptWords = (
      document.getElementById(CONFIG_MAX_PROMPT_WORDS)! as HTMLInputElement
    ).value;
    const completionCustomPrompt = (
      document.getElementById(
        CONFIG_COMPLETION_CUSTOM_PROMPT
      )! as HTMLInputElement
    ).value;
    const improvementCustomPrompt = (
      document.getElementById(
        CONFIG_IMPROVEMENT_CUSTOM_PROMPT
      )! as HTMLInputElement
    ).value;
    const disableCompletion = (
      document.getElementById(CONFIG_DISABLE_COMPLETION)! as HTMLInputElement
    ).checked;
    const disableImprovement = (
      document.getElementById(CONFIG_DISABLE_IMPROVEMENT)! as HTMLInputElement
    ).checked;
  ...
  document
    .getElementById(CONFIG_DISABLE_COMPLETION)
    ?.addEventListener('change', (e: Event) => {
      const checked = (e.target as HTMLInputElement).checked;
      (document.getElementById(CONFIG_MAX_TOKEN) as HTMLInputElement).disabled =
        checked;
      (
        document.getElementById(
          CONFIG_COMPLETION_CUSTOM_PROMPT
  ...
  document
    .getElementById(CONFIG_DISABLE_IMPROVEMENT)
    ?.addEventListener('change', (e: Event) => {
      const checked = (e.target as HTMLInputElement).checked;
      (
        document.getElementById(
